...
export async function query(
  { bindings, parent, selector }: QueryParams,
  ...
  if (selector.startsWith("pierce/")) {
    ...
    const opts = { nodeId: parent, selector };
...
export async function queryAll(
  ...
  if (selector.startsWith("pierce/")) {
    return await pierceQuerySelector({ bindings, parent, selector }, true);
  } else {
    const opts = { nodeId: parent, selector };
...
async function pierceQuerySelector(
  ...
  selector = selector.substring("pierce/".length);

  const { object } = await bindings.DOM.resolveNode({
    nodeId: parent,
  });

  // Calls `contentQuerySelectorPiercing(element, selector, queryAll)`
  // in content.
